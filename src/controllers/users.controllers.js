...
module.exports.signInUser = async (req, res, next) => {
    ...
    try {
        const userQuery = "SELECT username, password FROM users WHERE username = ?"
        db.query(userQuery, username, async (err, result) => {
            ...
            if (!pwdMatch) {
                return res.status(400).json({ message: "Authentification failed incorrect password" })
            }
            ...
            res.cookie("access_token", accessToken, { secure: false, httpOnly: false })
